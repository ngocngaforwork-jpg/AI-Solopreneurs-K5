...
const ZALO_GROUP_URL = "https://zalo.me/g/bop5bmsn5wkkyjkjg6za";
...
const RegistrationModal = ({ isOpen, onClose, utm }: { isOpen: boolean; onClose: () => void; utm: string }) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    submitData.append("timestamp", timestamp);
    ...
    try {
      ...
      const SCRIPT_URL = "https://script.google.com/macros/s/AKfycbx6hMeYhOIchMs9yy4CoQ3SwXkpvrC1GJ288-aoOWf8GiVuBf4EbU00xEpLEEuVZrqeJA/exec"; 
      
      if (SCRIPT_URL) {
        await fetch(SCRIPT_URL, {
          method: 'POST',
          body: submitData,
          mode: 'no-cors' // Sử dụng no-cors để tránh lỗi CORS policy
        });
      } else {
        console.warn("Chưa cấu hình SCRIPT_URL, dữ liệu chưa được gửi đi.");
      }
